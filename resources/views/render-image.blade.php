...
@if ($photo && $photoUrl)
    @if ($typeValue === 'img')
        <img style="@if ($widthValue)width:{{ $widthValue }} !important;@endif @if ($heightValue)height:{{ $heightValue }} !important;@endif @if (isset($style) && $style) {{ $style }} @endif" src="{{ url('/imgc/a4mw/'.$photoUrl) }}" class="{{ $classes }}" />
    @endif
